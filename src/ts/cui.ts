...
export class CUI {
    ...
    public static toObject<T, R>(array: T[], keyFn: (v: T) => any): R {
        let obj = {};
        let v: T;
        for (let i in array) {
            v = array[i];
     ...
     * @param data
     */
    public static comboboxToValueName<T extends Combobox | Combobox>(array: T[], render?: ValueNameRender): ValueName {
        try {
            let valueName: ValueName = { null: '無' };
            let data: T;
            for (let i in array) {
                data = array[i];
    ...
    public static isConnected(element: HTMLElement): boolean {
        if (element && element.parentElement) {
            if ((element as any).isConnected === undefined) {
                ...
                return (element as any).isConnected;
            }
        } else {
            return false;
        }
     ...
     * 不管例外发生
     * @param fn
     * @param value 发生例外时，返回预设值
     */
    public static ignoreError(fn: Function, value?: any): any {
        try {
            return fn();
        } catch (e) {
            return value;
